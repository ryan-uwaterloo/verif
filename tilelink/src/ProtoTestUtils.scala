...
object VerifBundleUtils {
  ...
  def MStatusHelper(debug: Bool = false.B, cease: Bool = false.B, wfi: Bool = false.B, isa: UInt = 0.U,
    ...
    def BoomDCacheReqHelper (addr: UInt = 0.U, data : Bits = 0.U, is_hella : Bool = false.B)(implicit p: Parameters): BoomDCacheReq = {
      new BoomDCacheReq().Lit(_.addr -> addr, _.data -> data, _.is_hella -> is_hella)
    }
  ...
  // def Gem5DCacheReqHelper (addr: UInt = 0.U, data: UInt = 0.U, is_hella: Bool = false.B) : BoomDCacheReq = {
  //   new BoomDCacheReq()
  // }
}
